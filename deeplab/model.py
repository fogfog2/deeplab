...
PROB_SUFFIX = '_prob'
SPATIAL_SCOPE = 'spatial_module'
ARM_SCOPE = 'arm_module'
BISE_SCOPE = 'bisenet'
FFM_SCOPE = 'ffm'
...
def extract_features(images,
  ...
  if not model_options.aspp_with_batch_norm:
    ...
    if model_options.dense_prediction_cell_config is not None:
      ...
      concat_logits = dense_prediction_layer.build__cell(
          features,
          output_stride=model_options.output_stride,
...
def bisnet_module(features,
  ...
  spatial_net = spatial_path_module(image,
      model_options,
      reuse,
      is_training,
      fine_tune_batch_norm,
  ...
  depth=21  
  net = feature_fusion_module(spatial_net, context_net, depth,
    model_options,
    reuse,
    is_training,
    fine_tune_batch_norm,
...
def feature_fusion_module(spatial_net, context_net, depth,
    ...
    weight_decay):
  branch_logits=[]
  branch_logits.append(spatial_net)
  branch_logits.append(context_net)
  
  concat_logits = tf.concat(branch_logits, 3)

  net = convblock2(concat_logits, 
      depth,
      model_options,
      reuse,
      is_training,
...
def ffm_sub_modlue(
  ...
  features_a = tf.identity(features,name='ffm')
...
def spatial_path_module( image,
      ...
      #spatial_path
  spatial_net = convblock1(image, 64,
      model_options,
      reuse,
      is_training,
      fine_tune_batch_norm,
  ...
  spatial_net = convblock1(spatial_net, 128,
      model_options,
      reuse,
      is_training,
      fine_tune_batch_norm,
  ...
  spatial_net = convblock1(spatial_net, 256,
      model_options,
      reuse,
      is_training,
      fine_tune_batch_norm,
...
def context_path_module(features,
      end_points,
      spatial_net,
      model_options,
      reuse,
  ...
  branch_logits.append(arm_8)
  branch_logits.append(arm_16)
  # Merge branch logits.
  concat_logits = tf.concat(branch_logits, 3)

  return concat_logits, arm_8, arm_16

def context_path_module_b(features,
      end_points,
      spatial_net,
      model_options,
      reuse,
...
def context_path_module_c(features,
      end_points,
      model_options,
      reuse,
      is_training,
...
def conv1by1_3(
  ...
  with slim.arg_scope(
        ...
        reuse=reuse,
        scope=scope):
    features = slim.conv2d(features, depth, 1)
                  
  return features


def convblock1(features, 
      depth,
      model_options,
      reuse,
      is_training,
